<template>
  <div class="talent-staff-list-container">
    <el-card>
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span class="text-lg font-medium">拟引进优秀人才候选人管理</span>
      ...
      <el-table :data="filteredStaffs" style="width: 100%" v-loading="loading" fit>
        ...
        <el-table-column prop="talentLevel" label="人才层次" />
        <el-table-column prop="proTitle" label="职称聘任" />
        <el-table-column prop="tutorQualify" label="导师资格" />
        ...
        <el-table-column
          ...
          label="人才层次投票数"
        />
        <el-table-column prop="proTitleNum" label="职称聘任投票数" />
        <el-table-column
          prop="tutorQualifyNum"
          label="导师资格投票数"
        />
    ...
    >
      ...
      >
        ...
        <el-form-item label="人才层次">
          <el-input v-model="editForm.talentLevel" placeholder="请输入人才层次" />
        </el-form-item>
        
        <el-form-item label="职称聘任">
          <el-input v-model="editForm.proTitle" placeholder="请输入职称聘任" />
        </el-form-item>
        
        <el-form-item label="导师资格">
          <el-input v-model="editForm.tutorQualify" placeholder="请输入导师资格" />
        </el-form-item>
...
const handleEditStaff = (staff) => {
  ...
  Object.assign(editForm, {
    id: staff.id,
    name: staff.name,
    talentLevel: staff.talentLevel,
    proTitle: staff.proTitle,
    tutorQualify: staff.tutorQualify,
    status: staff.status,
    ...
    isDelete: typeof staff.isDelete === 'undefined' ? 0 : Number(staff.isDelete) // 添加isDelete字段，0表示正常
  });
...
const handleSubmitEdit = async () => {
  try {
    ...
    const updateData = {
      id: editForm.id,
      name: editForm.name,
      talentLevel: editForm.talentLevel,
      proTitle: editForm.proTitle,
      tutorQualify: editForm.tutorQualify,
      ...
      isDelete: Number(editForm.isDelete) // 添加isDelete字段支持，确保为数字类型
    };
    
    loading.value = true;
    // 调用更新接口
    await updateTbEmp1(updateData);
...
const handleToggleStatus = async (staff) => {
  ...
  try {
    ...
    loading.value = true;
    // 调用更新接口
    await updateTbEmp1({ ...staff, status: newStatus });
...
const handleDeleteStaff = async (id) => {
  try {
    await ElMessageBox.confirm("确定要将该候选人标记为已删除吗？", "警告", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    });
