<template>
  <div class="guest-professor-vote-container">
    <el-card>
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span class="text-lg font-medium">客座教授引进意见投票管理</span>
...
import { getActivityEmp2s, addActivityEmp2, updateActivityEmp2, deleteActivityEmp2 } from '@/api/activityEmp2'
...
const handleSaveVote = async () => {
  ...
  try {
    ...
    if (editVoteFormData.activityId) {
      // 编辑现有投票
      await updateActivityEmp2(voteData)
      ElMessage.success('投票编辑成功')
    } else {
      // 添加新投票
      await addActivityEmp2(voteData)
...
const handleViewDetails = (vote) => {
  ...
  // 触发父组件的菜单切换事件，跳转到GuestMsg页面
  const event = new CustomEvent('menu-select', { detail: '4-2' });
...
<style scoped>
.guest-professor-vote-container {
  width: 100%;
}
