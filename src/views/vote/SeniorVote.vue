<template>
  <div class="container mx-auto p-4">
    ...
    <div class="mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-800 text-center">{{ activity.activityName }}</h1>
      <h2 class="text-lg text-gray-600 text-center mt-2">(名额{{ activity.maxVoteNum }}个)</h2>
      <div class="flex items-center mt-4 text-gray-500 text-sm justify-center">
        <span class="mr-4">投票状态：
          <span :class="activity.activityStatus === 1 ? 'text-green-600' : 'text-gray-400'">{{ activity.activityStatus === 1 ? '进行中' : '已结束' }}</span>
        </span>
    ...
    <div class="mb-4 bg-gray-50 p-4 rounded-lg">
      <div class="flex flex-wrap gap-4 items-end justify-center">
        <div class="flex flex-col">
          <label for="empName" class="text-sm text-gray-600 mb-1">教师姓名</label>
          <input 
            type="text" 
            id="empName" 
            v-model="empName" 
        ...
        <div class="flex flex-col">
          <label for="empCollege" class="text-sm text-gray-600 mb-1">所属学院</label>
          <input 
            type="text" 
            id="empCollege" 
            v-model="empCollege" 
    ...
    <div class="mb-6 bg-white shadow-sm rounded-lg overflow-hidden">
      ...
      <div v-if="rulesVisible" class="p-4 bg-blue-50/50 border border-blue-100">
        <ul class="text-sm text-gray-700 list-disc pl-5 space-y-1">
          <li>投票结果分为"同意"和"不同意"两个类别，其中"同意"分为A和B两个等级</li>
          <li>A等级为优先推荐、计3分，B等级为普通推荐、计1分；"不同意"为C，计0分</li>
      ...
      <table class="min-w-full border-collapse">
        <thead>
          <tr class="bg-gray-100">
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">序号</th>
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">姓名</th>
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">拟晋职称</th>
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">所在单位</th>
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">同意</th>
            <th class="border border-gray-300 px-4 py-2 text-sm text-gray-700">不同意</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(teacher, index) in teachers" :key="teacher.empId" class="hover:bg-gray-50">
            <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">{{ index + 1 }}</td>
            <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">{{ teacher.empName }}</td>
            <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">{{ teacher.empPrePosition }}</td>
            <td class="border border-gray-300 px-4 py-2 text-sm text-gray-700">{{ teacher.empCollege }}</td>
              ...
              <div class="flex space-x-4">
                <label class="flex items-center cursor-pointer">
                  <input 
                    ...
                    :disabled="!canVote || (voteSelections[teacher.empId] !== 'A' && selectedACount >= 4)"
                    class="mr-1"
                  >
                  <span>A</span>
                </label>
    ...
    <div class="mt-8 text-center">
      <button 
        @click="submitVote"
        :disabled="!canVote || (selectedACount + selectedBCount) === 0 || selectedACount > 4"
...
const selectedACount = computed(() => {
  return Object.values(voteSelections.value).filter(vote => vote === 'A').length
})

// 计算属性：已选择B票的数量
const selectedBCount = computed(() => {
  return Object.values(voteSelections.value).filter(vote => vote === 'B').length
})
...
const handleVoteSelection = (teacherId, voteType) => {
  ...
  if (voteType === 'A' && selectedACount.value > 4) {
    ...
    // 恢复之前的选择
    voteSelections.value[teacherId] = Object.keys(voteSelections.value).includes(teacherId) 
...
onMounted(() => {
  // 获取活动信息
  activity.value = {
    activityName: route.query.activityName || '2024年拟评正高级专业技术职务人员评审推荐投票',
